...
/usr/bin/codesign -s "Developer ID Application: University of Colorado Boulder (${2})" -vvvv --timestamp --options runtime --prefix 'edu.colorado.cires.cruisepack.app.' --keychain "$1" --force com/github/weisj/darklaf/platform/darklaf-macos/libdarklaf-macos-arm64.dylib

echo Signing sqlite dylib files
/usr/bin/codesign -s "Developer ID Application: University of Colorado Boulder (${2})" -vvvv --timestamp --options runtime --prefix 'edu.colorado.cires.cruisepack.app.' --keychain "$1" --force org/sqlite/native/Mac/aarch64/libsqlitejdbc.dylib
